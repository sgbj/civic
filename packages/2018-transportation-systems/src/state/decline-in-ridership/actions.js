import apiAdapter from '../api-adapter';
import actionEmitter from '../api-adapter-action-emitter';
import { getActiveRoute } from './selectors';

export const API_START = 'DECLINE_IN_RIDERSHIP_OVER_TIME/START';
export const API_SUCCESS = 'DECLINE_IN_RIDERSHIP_OVER_TIME/SUCCESS';
export const API_ERROR = 'DECLINE_IN_RIDERSHIP_OVER_TIME/ERROR';
export const SET_ROUTE = 'DECLINE_IN_RIDERSHIP_OVER_TIME/SET_ROUTE';

export const ridershipOverTimeStart = actionEmitter(API_START);
export const ridershipOverTimeSuccess = actionEmitter(API_SUCCESS);
export const ridershipOverTimeError = actionEmitter(API_ERROR);
export const ridershipOverTimeSetRoute = route => actionEmitter(SET_ROUTE)({route});

const DECLINE_IN_RIDERSHIP_API =
  'http://service.civicpdx.org/transportation-systems/passenger-census/routes/annual/averages/';

const routeEndpoint = route => `http://service.civicpdx.org/transportation-systems/passenger-census/routes/annual/averages/?route=${route}`

export const fetchRidershipOverTime = apiAdapter(DECLINE_IN_RIDERSHIP_API,{
  start: ridershipOverTimeStart,
  success: ridershipOverTimeSuccess,
  error: ridershipOverTimeError,
  buildParams: state => ({ route: getActiveRoute(state) }),
});
